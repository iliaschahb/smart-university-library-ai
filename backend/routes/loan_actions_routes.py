...
def create_loan():
    ...
    if not student:
        return jsonify({"error": f"Étudiant introuvable : {student_id}"}), 404
    ...
    if not inventory:
        return jsonify({"error": f"Stock introuvable pour le livre : {book_id}"}), 404
    ...
    db.session.add(loan)
    inventory.available_quantity = max(int(inventory.available_quantity or 0) - 1, 0)
    db.session.commit()
...
def return_loan(loan_id):
    ...
    if not loan:
        return jsonify({"error": f"Emprunt introuvable : {loan_id}"}), 404
...
def update_inventory(book_id):
    ...
    inventory = Inventory.query.filter_by(book_id=book_id).first()
    if not inventory:
        inventory = Inventory(book_id=book_id, quantity=0, available_quantity=0)
    ...
    return jsonify({
        ...
        "inventory": {
            "quantity": inventory.quantity,
            "available_quantity": inventory.available_quantity,
            "shelf_location": getattr(inventory, "shelf_location", None),
        },
        "analytics": {
            "local_loan_count": analytics.local_loan_count,
            "local_active_loan_count": analytics.local_active_loan_count,

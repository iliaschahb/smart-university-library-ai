...
def librarian_dashboard():
    ...
    active_loans = safe_int(
        db.session.query(func.count(LoanLocal.loan_id))
        .filter(LoanLocal.return_date.is_(None))
        .scalar()
    )
    returned_loans = safe_int(
        db.session.query(func.count(LoanLocal.loan_id))
        .filter(LoanLocal.return_date.isnot(None))
        .scalar()
    )
    ...
    top_borrowed_books = [
        {
            "book_id": row.book_id,
            "title": row.title,
            "loan_count": safe_int(row.loan_count)
        }
